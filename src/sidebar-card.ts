...
class SidebarCard extends LitElement {
  ...
  _runClock() {
    ...
    if (this.digitalClock && !this.twelveHourVersion) {
      ...
      var digitalTime =
        fullhours.length < 2 ? '0' + fullhours + ':' : fullhours + ':';
  ...
  updateSidebarSize(root) {
    ...
    if (sidebarInner) {
      ...
      if (this.config.debug === true) console.log('SIDEBAR-CARD: [updateSidebarSize] -> headerHeight', headerHeight);
  ...
  firstUpdated() {
    ...
    root.shadowRoot.querySelectorAll('paper-tab').forEach((paperTab) => {
      if (this.config.debug === true) console.log('SIDEBAR-CARD: [firstUpdated] -> Menu item found');
    ...
    if (this.bottomCard) {
      setTimeout(() => {
        ...
        if (this.config.debug === true) console.log('SIDEBAR-CARD: [firstUpdated] -> card',card);
...
function updateStyling(appLayout: any, sidebarConfig: any) {
  ...
  if (sidebarConfig.debug === true) console.group('SIDEBAR-CARD: [updateStyling]:');
  ...
  if (sidebarConfig.hideTopMenu && sidebarConfig.hideTopMenu === true && sidebarConfig.showTopMenuOnMobile && sidebarConfig.showTopMenuOnMobile === true && width <= sidebarConfig.breakpoints.mobile && offParam == null) {
    ...
    if (sidebarConfig.debug === true) console.log('Action: Show headerFooter!');
    if (hassFooter) { 
    ...
    if (sidebarConfig.debug === true) console.log('Action: Hide header and headerFooter!');
    if (hassHeader) {
  ...
  if (sidebarConfig.debug === true) console.groupEnd();

};

function subscribeEvens(
  appLayout: any,
  sidebarConfig,
  ...
  if ('hideOnPath' in sidebarConfig) {
    window.addEventListener('location-changed', () => {
      if (sidebarConfig.hideOnPath.includes(window.location.pathname)) {
        if (sidebarConfig.debug === true) console.log('SIDEBAR-CARD: [subscribeEvens] -> Sidebar disable for this path');
...
async function buildSidebar() {
  ...
  if (lovelace.config.sidebar) {
    ...
    if (
      ...
      // get element to wrap
      let contentContainer: any = appLayout.shadowRoot.querySelector(
        '#contentContainer'
      );
